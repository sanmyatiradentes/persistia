...
async function falar(texto, doisLocutores) {
  ...
  if (!r.ok) throw new Error('TTS HTTP ' + r.status + ': ' + (await r.text()).slice(0, 220));
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const cache = await db.execute({
      ...
      args: [String(topico_id), kind]
    });
    if (cache.rows.length) {
      return res.status(200).json({ audio_base64: cache.rows[0].dados, mime: 'audio/wav', cache: true });
    }

    const c = await db.execute({ sql: 'SELECT json FROM conteudos WHERE topico_id = ?', args: [String(topico_id)] });
    ...
    try {
      await db.execute({
        ...
        args: [String(topico_id), kind, wav, agora()]
      });
    } catch (_) {}
